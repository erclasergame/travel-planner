...
/**
 * Travel Planner - Base Data Population Script
 * Popola continenti e nazioni nel database Xata
 * 
 * PREREQUISITI:
 * - XATA_API_KEY nel .env
 * - XATA_DATABASE_URL nel .env
 * - npm install dotenv
 * 
 * USO: node travel_planner_populate_base_data.js
 */
...
if (!XATA_API_KEY || !XATA_DATABASE_URL) {
  console.error('❌ ERRORE: XATA_API_KEY e XATA_DATABASE_URL devono essere configurati nel .env');
  process.exit(1);
}

// 🌍 DATI CONTINENTI (6 continenti ufficiali)
const CONTINENTS = [
  { name: 'Europe', code: 'EU' },
  { name: 'Asia', code: 'AS' },
  { name: 'Africa', code: 'AF' },
...
// 🏳️ DATI PAESI (196 paesi ufficiali con continenti e bandiere)
const COUNTRIES = [
  // EUROPA
...
const makeRequest = async (endpoint, data, operation) => {
  const url = `${XATA_DATABASE_URL}/tables/${endpoint}/bulk`;
  
  log(`Inserimento ${operation}: ${data.length} record...`);
  ...
  log(`${operation} completato: ${result.results.length} record inseriti`, 'success');
...
const populateDatabase = async () => {
  log('🚀 Avvio popolamento database Travel Planner...');
  log(`📊 Dati da inserire: ${CONTINENTS.length} continenti, ${COUNTRIES.length} paesi`);

  try {
    // 1. Popolare continenti
    log('🌍 Popolamento continenti...');
    await makeRequest('continents', CONTINENTS, 'Continenti');

    // Pausa per evitare rate limit
    log('⏸️  Pausa 2 secondi...');
    await new Promise(resolve => setTimeout(resolve, 2000));

    // 2. Popolare paesi  
    log('🏳️ Popolamento paesi...');
    await makeRequest('countries', COUNTRIES, 'Paesi');

    log('🎉 Popolamento database completato!', 'success');
    log(`📈 Statistiche finali:`);
    log(`   • ${CONTINENTS.length} continenti inseriti`);
    log(`   • ${COUNTRIES.length} paesi inseriti`);
    log(`   • ${COUNTRIES.filter(c => c.continent_code === 'EU').length} paesi europei`);
    log(`   • ${COUNTRIES.filter(c => c.continent_code === 'AS').length} paesi asiatici`);
    log(`   • ${COUNTRIES.filter(c => c.continent_code === 'AF').length} paesi africani`);
    
    log('🔗 Verifica il database: /admin/database_quick', 'success');

  } catch (error) {
    log(`Errore durante il popolamento: ${error.message}`, 'error');
    process.exit(1);
  }
};
