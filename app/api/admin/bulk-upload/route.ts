...
export async function POST(request: NextRequest): Promise<NextResponse<BulkUploadResult>> {
  try {
    ...
    // URL endpoint Xata per insert
    const insertUrl = `${XATA_DATABASE_URL}/tables/${table}/data`;
    
    let inserted = 0;
    let updated = 0;
    const errors: string[] = [];

    // Processa ogni record singolarmente
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (response.ok) {
          // Successo
          inserted++;
          console.log(`✅ [Bulk Upload] Record ${i + 1} inserted successfully`);
        } else {
          ...
          try {
            ...
            errorMsg = `Record ${i + 1}: ${errorData.message || 'Errore sconosciuto'}`;
          } catch {
            errorMsg = `Record ${i + 1}: HTTP ${response.status} - ${responseText}`;
          }
...
export async function GET(): Promise<NextResponse> {
  return NextResponse.json({
    ...
    format: {
      ...
      data: 'array of objects (records da inserire)'
    },
    example: {
      table: 'countries',
      data: [
